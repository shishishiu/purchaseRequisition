...
public interface MemberRepository extends JpaRepository<MemberEntity, Long>
{
    ...
    @Modifying
    @Transactional
    @Query("UPDATE master_users SET password=?1, username=?2, department_id=?3, email=?4, is_deleted=?5, updated_user_id=?6 WHERE id=?7")
    void updateMember(String password, String username, int departmentId, 
    		String email, int isDeleted, String updateUserId, int id);
