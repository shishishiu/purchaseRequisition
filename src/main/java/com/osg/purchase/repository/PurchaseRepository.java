...
JpaSpecificationExecutor<PurchaseEntity>, CustomizedPurchaseRepository {
    ...
    void updatePurchaseHeader(String applicatedAt, String deliveryDate, int isDomestic, 
    		int company, String machineNo, String userId, int purchaseId);

    @Modifying
    @Transactional
    @Query("UPDATE table_purchases SET is_deleted=1, updated_user_id=?1 WHERE purchase_id=?2")
	public void updateIsDeletedDelete(String loggedInUserId, int purchaseId);
